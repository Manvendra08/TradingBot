...
async def lifespan(app: FastAPI):
    try:
        ...
        log.info(
            "Startup: Limited AnyIO default thread pool capacity to 15 to prevent thread/memory exhaustion"
        )
    ...
    def _warmup_instrument_cache():
        try:
            ...
            if kite:
                ...
                log.info(
                    "[startup] Kite not connected; instrument cache warm-up skipped."
                )
        except Exception as exc:
            log.warning("[startup] Instrument cache warm-up failed: %s", exc)

    threading.Thread(
        target=_warmup_instrument_cache, daemon=True, name="instrument-cache-warmup"
    ).start()
...
def _max_pain(rows: list[dict]) -> float | None:
    ce_map = {
        float(r["strike"]): int(r.get("oi") or 0)
        for r in rows
...
def _chain_context(
    rows: list[dict], underlying: float, prev_underlying: float | None
) -> dict:
    ce_rows = [r for r in rows if r.get("option_type") == "CE"]
    pe_rows = [r for r in rows if r.get("option_type") == "PE"]
    strikes = sorted(
        {float(r.get("strike") or 0) for r in rows if r.get("strike") is not None}
    )
    atm = (
        min(strikes, key=lambda s: abs(s - underlying))
        if strikes and underlying > 0
        else (strikes[0] if strikes else None)
    )
    ...
    if pe_rows:
        below = [
            r
            for r in pe_rows
            if r.get("strike") is not None and float(r["strike"]) <= underlying
        ]
        support = float(
            max((below or pe_rows), key=lambda r: int(r.get("oi") or 0))["strike"]
        )
    if ce_rows:
        above = [
            r
        ...
        resistance = float(
            max((above or ce_rows), key=lambda r: int(r.get("oi") or 0))["strike"]
        )
    if (
        support is not None
    ...
    if prev_underlying and prev_underlying != 0 and underlying > 0:
        price_change_pct = round(
            (underlying - prev_underlying) / abs(prev_underlying) * 100, 4
        )
...
def _fetch_natgas_news() -> dict:
    ...
    try:
        res = requests.get(
            _TV_NEWS_API, timeout=20, headers={"User-Agent": "Mozilla/5.0"}
        )
        ...
        return {
            ...
            "potential_news_direction": "MIXED",
        }
...
def _fetch_scanx_heatmap(symbol: str) -> dict:
    ...
    try:
        ...
        top = sorted(
            rows, key=lambda x: abs(_safe_float(x.get("PPerchange"), 0.0)), reverse=True
        )[:8]
        out = {
            "total": len(rows),
            "adv": adv,
...
def _chart_dir_score(chart_payload: dict) -> float:
    ...
    for tf in ("1h", "3h"):
        ...
        if isinstance(tf_data, dict):
            vals.append(
                score_map.get(str(tf_data.get("sentiment") or "NEUTRAL").upper(), 0.0)
            )
...
def get_topbar_cmps():
    ...
    for sym in symbols:
        ...
        latest_sym = sym
        row = _q(
            "SELECT symbol, price FROM underlying_price WHERE symbol=? ORDER BY fetched_at DESC LIMIT 1",
            (sym,),
        )
        if row:
            ...
            row2 = _q(
                "SELECT symbol, price FROM underlying_price WHERE symbol LIKE ? ORDER BY fetched_at DESC LIMIT 20",
                (f"{sym}%",),
            )
            for r in row2:
...
def get_intelligence_summary(symbol: str):
    ...
    prev_underlying = (
        _safe_float((up_rows[1] if len(up_rows) > 1 else {}).get("price"), 0.0) or None
    )
    ...
    out = {
        ...
        "summary_lines": intel.get("summary_lines")
        or [f"Verdict: {intel.get('verdict', 'UNKNOWN')}"],
    ...
    if scan_frequency_minutes is not None:
        if scan_frequency_minutes not in ALLOWED_SCAN_FREQUENCIES:
            return JSONResponse(
                {
                    "ok": False,
                    "error": "invalid scan_frequency_minutes",
                    "allowed": ALLOWED_SCAN_FREQUENCIES,
                },
                status_code=400,
            )
    ...
    if scan_frequency_nse is not None:
        if scan_frequency_nse not in ALLOWED_SCAN_FREQUENCIES:
            return JSONResponse(
                {
                    "ok": False,
                    "error": "invalid scan_frequency_nse",
                    "allowed": ALLOWED_SCAN_FREQUENCIES,
                },
                status_code=400,
            )
    ...
    if scan_frequency_mcx is not None:
        if scan_frequency_mcx not in ALLOWED_SCAN_FREQUENCIES:
            return JSONResponse(
                {
                    "ok": False,
                    "error": "invalid scan_frequency_mcx",
                    "allowed": ALLOWED_SCAN_FREQUENCIES,
                },
                status_code=400,
            )
...
def _enrich_open_trades_with_live_pnl(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        if option_type == "FUT" or not option_type or strike is None:
            res = _q(
                "SELECT price FROM underlying_price WHERE symbol=? ORDER BY fetched_at DESC LIMIT 1",
                (symbol,),
            )
            if res:
                cmp = res[0]["price"]
        ...
        if cmp is not None and cmp > 0:
            if option_type == "FUT":
                ...
                entry = float(
                    row.get("entry_premium") or row.get("entry_underlying") or 0.0
                )
...
def _enrich_trade_details(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        is_tf = (row.get("setup_type") == "TIMEFRAME") or (
            "timeframe" in str(row.get("reason") or "").lower()
        )
        if is_tf:
        ...
        row["verdict_explanation"] = _explain_verdict(
            row.get("verdict_label"), row.get("option_type")
        )
...
async def get_paper_trades(symbol: str = "", status: str = "", limit: int = 300):
    ...
    for ml in ml_rows:
        ...
        rows.append({
            ...
            "reason": ml.get("entry_reason") or (ml.get("reason") if ml.get("status") == "OPEN" else "") or "; ".join(l.get("rationale") for l in legs if l.get("rationale")) or "—",
            "entry_reason": ml.get("entry_reason") or (ml.get("reason") if ml.get("status") == "OPEN" else "") or "; ".join(l.get("rationale") for l in legs if l.get("rationale")) or "—",
            "exit_reason": ml.get("exit_reason") or (ml.get("reason") if ml.get("status") != "OPEN" else "") or "",
...
def _explain_verdict(verdict: str | None, option_type: str | None) -> dict:
    ...
    explanations = {
        ...
        "Put Writing": {
            ...
            "action": "Legacy CE proxy; current engine skips writing trades"
            if ot == "CE"
            else "Sell PE",
            "emoji": "📗",
        },
        "Call Writing": {
            ...
            "action": "Legacy PE proxy; current engine skips writing trades"
            if ot == "PE"
            else "Sell CE",
            "emoji": "📕",
        },
...
async def get_paper_summary(symbol: str = ""):
    ...
    for ml in ml_rows:
        ...
        ml_trade_dict = {
            ...
            "pnl_points": 0.0,
            "status": st,
            "reason": ml.get("entry_reason") or (ml.get("reason") if st == "OPEN" else "") or "; ".join(l.get("rationale") for l in legs if l.get("rationale")) or "—",
            "entry_reason": ml.get("entry_reason") or (ml.get("reason") if st == "OPEN" else "") or "; ".join(l.get("rationale") for l in legs if l.get("rationale")) or "—",
            "exit_reason": ml.get("exit_reason") or (ml.get("reason") if st != "OPEN" else "") or "",
            "digest_id": ml.get("digest_id"),
...
def _calculate_holding_analysis(where: str, params: tuple) -> dict:
    ...
    return {
        ...
        "distribution_pct": {
            "under_5min": round((under_5 / len(durations)) * 100, 1)
            if durations
            else 0,
            "5_to_15min": round((five_to_15 / len(durations)) * 100, 1)
            if durations
            else 0,
            "15_to_30min": round((fifteen_to_30 / len(durations)) * 100, 1)
            if durations
            else 0,
            "30_to_60min": round((thirty_to_60 / len(durations)) * 100, 1)
            if durations
            else 0,
            "over_60min": round((over_60 / len(durations)) * 100, 1)
            if durations
            else 0,
        },
...
async def manual_close_paper_trade(trade_id: int = Query(...)):
    ...
    from src.models.schema import close_live_trade, close_paper_trade, get_conn
    ...
    if row.get("status") != "OPEN":
        return JSONResponse(
            {"ok": False, "error": "Trade is already closed"}, status_code=400
        )
    ...
    with get_conn() as conn:
        res = conn.execute(
            "SELECT 1 FROM live_trades WHERE id=?", (trade_id,)
        ).fetchone()
        if res:
            is_live = True
...
async def delete_paper_trades(date_from: str = "", date_to: str = ""):
    ...
    if not date_from and not date_to:
        return JSONResponse(
            {"ok": False, "error": "Provide at least date_from or date_to"},
            status_code=400,
        )
    clauses = []
    params: list = []
...
def _fetch_real_kite_positions(kite) -> list[dict]:
    ...
    try:
        ...
        try:
            ...
            log.warning(
                "Failed to fetch GTTs from Kite in _fetch_real_kite_positions: %s", ge
            )

        open_db_trades = _q(
            "SELECT * FROM live_trades WHERE status='OPEN' AND (trade_status IS NULL OR trade_status != 'SHADOW')"
        )
        db_map = {}
        db_fallback_map = {}
        for t in open_db_trades:
            ...
            if expiry_val:
                resolved = resolve_instrument(
                    t["symbol"], expiry_val, strike_val or 0.0, t["option_type"]
                )
        ...
        for pos in net_positions:
            ...
            if expiry_val and expiry_val != "—":
                try:
                    res_inst = resolve_instrument(
                        symbol, expiry_val, strike or 0.0, option_type
                    )
            ...
            if not db_trade:
                candidate_db_trade = db_fallback_map.get(
                    (symbol, option_type, strike, side.upper())
                )
            ...
            if db_trade:
                ...
                sl_val = (
                    db_trade.get("sl_premium") or db_trade.get("sl_underlying") or "—"
                )
                tgt_val = (
                    db_trade.get("target_premium")
                    or db_trade.get("target_underlying")
                    or "—"
                )
                ...
                if db_side == side and abs(qty) > db_qty:
                    ...
                    if option_type == "FUT":
                        ...
                        bot_entry = float(
                            db_trade.get("entry_premium")
                            or db_trade.get("entry_underlying")
                            or entry_px
                        )

                    bot_pnl = (
                        (cmp - bot_entry) * db_lots * lot_size
                        if side == "BUY"
                        else (bot_entry - cmp) * db_lots * lot_size
                    )
...
def _get_kite_closed_trades(kite) -> list[dict]:
    ...
    for tsym in completed_orders_map:
        completed_orders_map[tsym].sort(
            key=lambda x: str(x.get("order_timestamp") or "")
        )
...
def _is_duplicate_closed_trade(kite_pos, db_trades, today_str) -> bool:
    ...
    for t in db_trades:
        ...
        if db_expiry:
            try:
                resolved = resolve_instrument(
                    db_sym, db_expiry, db_strike or 0.0, db_opt
                )
                if resolved:
        ...
        if db_tsym and ktsym:
            ...
            if db_sym == k_sym and db_opt == k_opt:
                if (
                    k_strike is None
                    or db_strike is None
                    or abs(db_strike - k_strike) < 0.01
                ):
                    return True
    return False
    ...
    if status.upper() == "OPEN":
        ...
        if kite:
            ...
            real_positions = [
                ...
                and float(p.get("lots") or p.get("quantity", 0)) != 0
            ]
        ...
        if shadow_mode:
            ...
            real_tsyms = {
                (p["tradingsymbol"].upper(), p["side"].upper())
                for p in real_positions
            ...
            real_keys = {
                (
                    p["symbol"].upper(),
                    p["option_type"].upper(),
                    float(p["strike"]) if p.get("strike") is not None else None,
                    p["side"].upper(),
                )
                for p in real_positions
            ...
            for row in db_rows:
                strike_val = (
                    float(row["strike"]) if row.get("strike") is not None else None
                )
                expiry_val = row.get("expiry") or ""
                resolved = None
                if expiry_val:
                    resolved = resolve_instrument(
                        row["symbol"], expiry_val, strike_val or 0.0, row["option_type"]
                    )

                db_tsym = (
                    resolved["tradingsymbol"].upper()
                    if (resolved and resolved.get("tradingsymbol"))
                    else ""
                )
        ...
        if kite:
            try:
                ...
                if symbol:
                    sym_upper = symbol.upper().strip()
                    unique_kite_closed = [
                        kp for kp in unique_kite_closed if kp["symbol"] == sym_upper
                    ]
                ...
                log.error(
                    "Failed to merge closed Kite positions in live_trades: %s", ke
                )
...
def get_portfolio_metrics():
    ...
    for p in open_positions:
        ...
        if sym.startswith("NATURALGAS"):
            ...
            base = "NIFTY"
        elif sym.startswith("BANKNIFTY"):
            base = "BANKNIFTY"
        elif sym.startswith("CRUDEOIL"):
            base = "CRUDEOIL"
        elif sym.startswith("GOLD"):
            base = "GOLD"
    ...
    for base, positions in groups.items():
        ...
        rows = _q(
            "SELECT * FROM option_chain_snapshots WHERE symbol=? ORDER BY id DESC LIMIT 200",
            (base,),
        )
        if rows:
                    ...
                    if rows:
                        match = next(
                            (
                                r
                                for r in rows
                                if abs(float(r["strike"] or 0) - strike) < 0.01
                                and r["option_type"] == opt_type
                            ),
                            None,
                        )
        ...
        for S in prices:
            ...
            for p in positions:
                ...
                if opt_type == "FUT":
                    entry = float(
                        p.get("entry_underlying") or p.get("entry_premium") or 0
                    )
        ...
        has_options = any(
            (p.get("option_type") or "").upper() in ("CE", "PE") for p in positions
        )
...
def get_risk_metrics(mode: str = "live"):
    ...
    from config.settings import LOT_SIZES, MAX_DAILY_LOSS_RUPEES
    from src.models.schema import get_conn
    ...
    if mode == "live":
        ...
        if kite:
            try:
                ...
                log.error(
                    "Failed to merge closed trades for Profit Factor and realized PnL: %s",
                    e,
                )
    ...
    profit_factor = (
        ...
        if total_losses > 0
        else (99.99 if total_wins > 0 else 0.0)
    )
    ...
    daily_loss_limit = float(
        runtime_config.get("live_max_daily_loss_rupees", MAX_DAILY_LOSS_RUPEES)
    )
            ...
            def get_avail(sec):
                ...
                if isinstance(avail, dict):
                    return float(
                        avail.get(
                            "live_balance",
                            avail.get("cash", avail.get("opening_balance", 0.0)),
                        )
                    )
                return 0.0
...
def zerodha_callback(client_id: str = None, request_token: str = None):
    ...
    try:
        ...
        try:
            ...
            log.warning(
                "Failed to configure TLS adapter for Zerodha callback: %s", ssl_err
            )
        ...
        for attempt in range(1, 4):
            try:
                ...
                if "token is invalid" in err_msg or "token" in err_msg:
                    break
                log.warning(
                    "generate_session attempt %d/3 failed: %s. Retrying in 1s...",
                    attempt,
                    err,
                )
                time.sleep(1)
        ...
        today = datetime.now(timezone(timedelta(hours=5, minutes=30))).strftime(
            "%Y-%m-%d"
        )
        ...
        return HTMLResponse(
            f"<h1>Failed to generate Zerodha session: {e}</h1>", status_code=500
        )
...
async def zerodha_postback(request: Request):
    ...
    from src.models.schema import close_live_trade, get_broker_config, get_conn
    ...
    if not order_id or not order_timestamp or not status or not checksum:
        return JSONResponse(
            {"error": "Missing required postback fields"}, status_code=400
        )

    expected = hashlib.sha256(
        f"{order_id}{order_timestamp}{config['api_secret']}".encode("utf-8")
    ).hexdigest()
    ...
    with get_conn() as conn:
        trade = conn.execute(
            "SELECT * FROM live_trades WHERE broker_order_id=? AND status='OPEN'",
            (order_id,),
        ).fetchone()
        if trade:
            trade = dict(trade)
            if status in ("REJECTED", "CANCELLED"):
                ...
                log.info(
                    "Live trade %s closed because entry order was %s",
                    trade["id"],
                    status,
                )
            elif status == "COMPLETE":
                conn.execute(
                    "UPDATE live_trades SET broker_status='COMPLETE' WHERE id=?",
                    (trade["id"],),
                )
        ...
        if payload.get("gtt_id"):
            trade = conn.execute(
                "SELECT * FROM live_trades WHERE gtt_order_id=? AND status='OPEN'",
                (str(payload.get("gtt_id")),),
            ).fetchone()
        ...
        if not trade and status == "COMPLETE" and tradingsymbol:
            ...
            for ot in open_trades:
                ot = dict(ot)
                resolved = resolve_instrument(
                    ot["symbol"], ot["expiry"], ot["strike"], ot["option_type"]
                )
            ...
            if len(matches) == 1:
                ...
                log.warning(
                    "Ambiguous Zerodha exit postback for %s matched %d open trades",
                    tradingsymbol,
                    len(matches),
                )
        ...
        if trade and status == "COMPLETE":
            trade = dict(trade)
            exit_premium = float(
                payload.get("average_price") or payload.get("price") or 0.0
            )
            res_u = conn.execute(
                "SELECT price FROM underlying_price WHERE symbol=? ORDER BY fetched_at DESC LIMIT 1",
                (trade["symbol"],),
...
async def ai_insights_page():
    html_path = ROOT / "src" / "dashboard" / "ai.html"
    if html_path.exists():
        return HTMLResponse(html_path.read_text(encoding="utf-8"))
