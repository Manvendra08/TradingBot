...
async def get_paper_trades(symbol: str = "", status: str = "", limit: int = 300):
    ...
    for ml in ml_rows:
        ...
        rows.append({
            ...
            "reason": ml.get("entry_reason") or (ml.get("reason") if ml.get("status") == "OPEN" else "") or "—",
            "entry_reason": ml.get("entry_reason") or (ml.get("reason") if ml.get("status") == "OPEN" else "") or "—",
