...
from config.settings import (
    ...
    OTM_STRIKE_RANGE,
    OTM_OI_SPIKE_PCT,
    VOLUME_AGGRESSION_HIGH,
    VOLUME_AGGRESSION_LOW,
    IV_CRUSH_THRESHOLD,
    STRADDLE_DELTA_PCT,
    ATM_LEG_MOVE_PCT,
    PCR_VELOCITY_WINDOW,
...
def _oi_wall(strikes_data: list[dict]) -> dict:
    ...
    if support == resistance:
        ...
        alt_sup = next(
            (r["strike"] for r in pe_sorted if r["strike"] != resistance), None
        )
...
def _key_levels(strikes_data: list[dict], underlying: float) -> dict:
    ...
    ce_rows = [
        ...
        if r.get("option_type") == "CE" and (r.get("oi") or 0) > 0
    ]
    pe_rows = [
        r
        for r in strikes_data
        if r.get("option_type") == "PE" and (r.get("oi") or 0) > 0
    ]
    support = None
    resistance = None
...
def _detect_buildup(
    ...
    for row in filtered:
        ...
        alerts.append(
            _make_alert("BUILDUP_CLASSIFY", symbol, expiry, strike, ot, detail, sev)
        )
    return alerts
...
def _detect_price_spike(symbol: str, expiry: str, underlying: float) -> list[dict]:
    ...
    log.info(
        "[engine] PRICE_SPIKE | %s: %.2f\u2192%.2f (%.2f%%) [%s]",
        symbol,
        prev_price,
        underlying,
        pct,
...
def _detect_pcr_velocity(
    symbol: str, expiry: str, underlying: float, curr_pcr: float | None
) -> list[dict]:
    if curr_pcr is None:
        return []
    ...
    # Allow 2-element series (curr + 1 previous) for basic velocity check.
    # Original guard len < 3 was too strict: if snapshots had 1 valid PCR
    # (plus curr_pcr = 2 total), velocity was silently skipped.
    if len(pcr_series) < 2:
        return []
...
def _detect_iv_spike_crush(
    ...
    for ot in ("CE", "PE"):
        row = next(
            (r for r in filtered if r["strike"] == atm and r["option_type"] == ot), None
        )
        if not row:
            continue
        ...
        if delta >= IV_SPIKE_ATM_THRESHOLD:
            ...
            detail = {
                ...
                "prev_iv": prev_iv,
                "curr_iv": curr_iv,
                "iv_delta": round(delta, 2),
                "underlying": underlying,
            }
            log.info(
                "[engine] IV_SPIKE | %s ATM %.0f %s: %.1f\u2192%.1f [%s]",
                symbol,
                atm,
                ot,
                prev_iv,
            ...
            log.info(
                "[engine] IV_CRUSH | %s ATM %.0f %s: %.1f\u2192%.1f",
                symbol,
                atm,
                ot,
                prev_iv,
            ...
            alerts.append(
                _make_alert("IV_CRUSH", symbol, expiry, atm, ot, detail, "MEDIUM")
            )
    return alerts


def _detect_straddle_premium(
    filtered: list[dict],
    symbol: str,
    expiry: str,
    underlying: float,
    ...
    atm = _atm_strike(filtered, underlying)
    ce_row = next(
        (r for r in filtered if r["strike"] == atm and r["option_type"] == "CE"), None
    )
    pe_row = next(
        (r for r in filtered if r["strike"] == atm and r["option_type"] == "PE"), None
    )
...
def _detect_max_pain_shift(
    ...
    if prev_mp and abs(max_pain - prev_mp) >= MAX_PAIN_SHIFT_THRESHOLD:
        ...
        detail = {
            ...
            "shift": max_pain - prev_mp,
            "underlying": underlying,
        }
        log.info(
            "[engine] MAX_PAIN_SHIFT | %s: %.0f\u2192%.0f [%s]",
            symbol,
            prev_mp,
            max_pain,
            sev,
        ...
        return [
            _make_alert("MAX_PAIN_SHIFT", symbol, expiry, max_pain, None, detail, sev)
        ]
    return []


def _detect_oi_wall_shift(
    ...
    changes = {}
    if (
        curr_wall["resistance"]
        and prev_wall["resistance"]
        and curr_wall["resistance"] != prev_wall["resistance"]
    ):
        changes["resistance"] = {
            "prev": prev_wall["resistance"],
            "curr": curr_wall["resistance"],
        }
    if (
        curr_wall["support"]
        and prev_wall["support"]
        and curr_wall["support"] != prev_wall["support"]
    ):
        changes["support"] = {
            "prev": prev_wall["support"],
...
def _detect_otm_unusual(
    ...
    for row in otm:
        ...
        alerts.append(
            _make_alert("OTM_UNUSUAL", symbol, expiry, strike, ot, detail, sev)
        )
    return alerts
...
def detect_anomalies(
    ...
    if not strikes:
        ...
        return [], _empty_scan_context(
            symbol, expiry, underlying, chart_indicators, "empty_strikes"
        )
    ...
    for ot in ("CE", "PE"):
        ...
        if row and prev:
            ...
            if pct is not None:
                moves[ot] = {"pct": pct}
    if moves:
        ce_p = moves.get("CE", {}).get("pct", 0)
        pe_p = moves.get("PE", {}).get("pct", 0)
        if abs(ce_p) >= ltp_thresh or abs(pe_p) >= ltp_thresh:
            ...
            alerts.append(
                _make_alert(
                    ...
                    atm,
                    None,
                    {"ce_pct": ce_p, "pe_pct": pe_p, "bias": bias},
                    sev,
                )
            )
    ...
    mp_change = (
        (curr_mp - prev_mp) if (curr_mp is not None and prev_mp is not None) else 0.0
    )
