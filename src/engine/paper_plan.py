...
def build_paper_trade_plan(verdict: str, confidence: int, ctx: dict) -> dict | None:
    ...
    if option_type in ("CE", "PE"):
        if side == "SELL":
            if option_type == "CE":
                strike = _round_to_step(resistance, step) if (resistance and resistance > underlying) else _round_to_step(underlying + step, step)
            else:
                strike = _round_to_step(support, step) if (support and support < underlying) else _round_to_step(underlying - step, step)
        else:
            strike = atm
    else:
        # FUT
