...
class TestPipelineIntegration:
    ...
    def test_pipeline_dedup_suppresses_repeat_alert(self, sample_oc_nifty):
        ...
        def count_alerts():
            return len(
                [
                    a
                    for a in get_alert_history("NIFTY", limit=200)
    ...
    def test_pipeline_continues_when_process_symbol_crashes(self):
        with patch(
            "src.engine.pipeline._process_symbol",
            side_effect=Exception("Symbol process crash"),
        ):
            from src.engine.pipeline import run_pipeline
    ...
    def test_pipeline_continues_when_save_scan_summary_fails(self, sample_oc_nifty):
        with (
            ...
            patch(
                "src.engine.pipeline.save_scan_summary",
                side_effect=Exception("Save summary failed"),
            ),
        ):
            from src.engine.pipeline import run_pipeline
...
class TestFetcherRouter:
    ...
    def test_router_falls_back_on_none(self):
        ...
        with patch(
            "src.fetchers.router._priority_for", return_value=["dhan", "nse_public"]
        ):
...
class TestTelegramFormatter:
    ...
    def test_oi_spike_message_contains_key_fields(self):
        ...
        assert "22000" in msg
        assert any(
            x in msg for x in ("1.0L", "1.00L", "1.4L", "1.40L", "100000", "1,00,000")
        )
    ...
    def test_max_pain_message(self):
        ...
        alert = self._make_alert(
            "MAX_PAIN_SHIFT",
            {
                "prev_max_pain": 21900.0,
                "curr_max_pain": 22100.0,
                "shift": 200.0,
                "underlying": 22050.0,
            },
        )
...
class TestSchedulerMarketHours:
    ...
    def test_guarded_run_all_closed(self):
        ...
        with (
            patch("src.scheduler.job_runner._is_open_for", return_value=False),
    ...
    def test_guarded_run_some_open(self):
        ...
        with (
            patch("src.scheduler.job_runner._is_open_for", side_effect=side_effect),
            patch("src.scheduler.job_runner.run_pipeline") as mock_pipeline,
        ):
            _guarded_run()
    ...
    def test_run_dhan_naturalgas_scrape_success(self):
        ...
        with (
            patch("src.scheduler.job_runner.SCRAPE_RUNNER") as mock_runner,
