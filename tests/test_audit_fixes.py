...
class TestLiveReversalGuard:
    ...
    def test_live_reversal_requires_all_three_guards(self):
        ...
        with get_conn() as conn:
            ...
            """, (
                datetime.now(timezone.utc).isoformat(), "NIFTY", "2026-06-25",
                "Long Buildup", "BUY", "CE", 22000.0, 22000.0, 100.0,
                21900.0, 70.0, 22200.0, 150.0, 1, "OPEN", "test",
                "digest-test", "TRIGGERED_CORE", "TREND_CONTINUATION",
                "test", "signal-key-1", "order-1", "COMPLETE", "GTT"
            ))
        ...
        with patch("src.engine.live_trading._is_market_open", return_value=True), \
             patch("src.engine.live_trading.load_runtime_config", return_value=runtime_config), \
...
class TestPaperMonitorPremiumSl:
    ...
    def test_buy_ce_premium_sl_hit(self):
        ...
        with get_conn() as conn:
            ...
            """, (
                datetime.now(timezone.utc).isoformat(), "NIFTY", "Long Buildup",
                "CE", 22000.0, 22000.0, 200.0, 150.0, 300.0,
                21800.0, 22400.0, "OPEN", "CORE", 1, "BUY"
            ))
        ...
        with get_conn() as conn:
            row = conn.execute("SELECT status, exit_premium FROM paper_trades WHERE id=?", (trade_id,)).fetchone()
...
class TestAtrBreakoutBuffer:
    ...
    def test_breakout_buffer_uses_atr_when_available(self):
        ...
        with patch("src.engine.paper_trading._is_market_open", return_value=True), \
            ...
            result = run_timeframe_strategy("NIFTY", ctx, "digest-test", {"verdict_label": "Long Buildup", "confidence": 80})
...
class TestTimeWeightedRegimeDecay:
    ...
    def test_old_scans_have_lower_weight(self):
        ...
        from src.engine.regime_detector import detect_market_regime, REGIME_TRENDING_UP
