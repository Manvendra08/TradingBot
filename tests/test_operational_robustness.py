...
def test_pnl_fallback_time_value():
    ...
    with get_conn() as conn:
        ...
        trade_id = conn.execute(
            "SELECT id FROM paper_trades WHERE symbol='NATURALGAS'"
        ).fetchone()["id"]
    ...
    close_paper_trade(
        trade_id, "2026-06-10T10:10:00", 152.0, None, "CLOSED_TARGET", "target hit"
    )
...
def test_underlying_cmp_prioritized_exit():
    ...
    monitor_paper_trades(
        ...
        {
            "underlying": 161.0,
            "option_rows": [{"strike": 150.0, "option_type": "CE", "ltp": 12.0}],
        },
    )
    ...
    with get_conn() as conn:
        ...
        # 0.5% slippage applied (12.0 * 0.995 = 11.94)
        assert trade["exit_premium"] == 12.0 * 0.995
...
def test_build_paper_trade_plan_fut_atr():
    ...
    # Timeframe FUT trade should bypass ATR SL/TP logic (default to step-based fallback)
    ...
    # Without setup_type check in build_paper_trade_plan, ATR is used for all trades.
    # Verify it still computes valid ATR-based values.
