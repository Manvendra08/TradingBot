...
# ASGITransport in httpx 0.28+ only has handle_async_request.
# FastAPI TestClient uses sync Client which expects a sync handle_request.
# Add a sync wrapper that runs the async handler via anyio.
_orig_asgi_transport_init = ASGITransport.__init__


def _patched_asgi_init(self, app, *args, **kwargs):
    _orig_asgi_transport_init(self, app, *args, **kwargs)
    # Attach a sync handle_request that delegates to handle_async_request
    if not hasattr(self, "handle_request"):
        import anyio

        async def _handle_async(request):
            return await self.handle_async_request(request)

        def handle_request(request):
            return anyio.run(_handle_async, request)

        self.handle_request = handle_request


ASGITransport.__init__ = _patched_asgi_init
...
def isolated_db():
    ...
    with (
        patch("src.models.schema.DB_PATH", tmp_path),
        patch("config.settings.DB_PATH", tmp_path),
    ):
        import sys

        for mod_name in [
            "src.fetchers.chart_fetcher",
            "dashboard_server",
            "src.dashboard.app",
        ]:
...
def no_telegram(monkeypatch):
    ...
    monkeypatch.setattr(
        "src.alerts.telegram_dispatcher.send_alert", lambda alert: False
    )
...
def mock_llm_calls(request):
    ...
    if (
        request.cls and request.cls.__name__ == "TestOpenRouterArrayUnwrap"
    ) or request.node.name == "test_llm_alternative_fallbacks":
        yield
        return

    from src.engine.llm_enrichment import (
        LLMExitAdvice,
        LLMStrategyOptimization,
        LLMTradeVerdict,
    )
    ...
    def mock_call(symbol, prompt, response_schema=None, deadline=None):
        ...
        if schema == LLMExitAdvice:
            ...
            return LLMTradeVerdict(
                action="NO_TRADE",
                confidence=50,
                signal_chain="OI: Test pattern — CE Δ 0 vs PE Δ 0 → NEUTRAL\nPrice: flat vs ATM → no edge\nChart: 1H NEUTRAL + 3H NEUTRAL → no timing",
...
try:
    ...
    class MockGeminiModels:
        def generate_content(self, *args, **kwargs):
            raise RuntimeError(
                "Real Gemini call blocked in tests to prevent token usage"
            )
